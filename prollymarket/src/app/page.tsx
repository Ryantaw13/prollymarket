  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-600">Trade on anything with fake money. No risk, just practice.</p>
      </div>
            ...
            <Link key={market.id} href={`/market/${market.id}`} className="card hover:shadow-md transition-shadow">
              ...
              <div className="flex gap-4">
                <div className="flex-1 bg-green-50 rounded-lg p-3 text-center">
                  ...
                  <div className="text-green-600 font-bold">{formatPrice(market.yesPrice)}</div>
                </div>
                <div className="flex-1 bg-red-50 rounded-lg p-3 text-center">
                  <div className="text-xs text-gray-500 mb-1">NO</div>
                  <div className="text-red-600 font-bold">{formatPrice(market.noPrice)}</div>
                </div>
              </div>
              {market.isResolved && (
                <div className={`mt-3 text-sm font-medium ${market.outcome === 'YES' ? 'text-green-600' : 'text-red-600'}`}>
